...
def Colorize(text: str = "", foreground: Foreground = Foreground.DEFAULT, background: Background = Background.NOTHING, style: Modifier = Modifier.NOTHING):
...
def print_options():
    ...
    for color in Foreground:
        list.append([Colorize(foreground=color, style=style)
                    for style in Modifier])
        list[-1].append(Colorize(text=color.name, foreground=color))

    for color in Background:
        list.append([Colorize(background=color, style=style)
                    for style in Modifier])
